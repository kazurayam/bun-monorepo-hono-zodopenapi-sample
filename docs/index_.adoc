...
```
$ cd $ROOT; bun run backend
```

```
$ cd $ROOT; bun run frontend
```
...
- http://localhost:5173/api/users でbackendによるAPIサービスが動作していることを見ることができる。
- http://localhost:5173/doc/ui でSwagger UIが生成したAPIドキュメントを見ることができる
- http://localhost:3000/ でfrontendによるwebアプリが動作していることを見ることができる。
